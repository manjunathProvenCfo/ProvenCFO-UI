...
(function ($, _, undefined) {
    ...
    var defaultSettings = {
        ...
        templates: {
            ...
            mentionItemHighlightAnchor: _.template('<strong><span></span></strong>')
        }
    };
    ...
    var MentionsInput = function (settings) {
        ...
        function initTextarea() {
            ...
            //elmInputWrapper = elmInputBox.parents('.chat-editor-area').find('.emojionearea'); //Get the DOM element parent
            elmInputWrapper = elmInputBox.parents(); //Get the DOM element parent
            elmWrapperBox = $(settings.templates.wrapper());
            elmInputBox.parents("#message-body-div").wrapAll(elmWrapperBox); //Wrap all the text area into the div elmWrapperBox
            ...
            elmInputBox.bind('change', onInputBoxInput); //Bind the blur event to the text area

            $chatEditorArea[0].emojioneArea.on("keydown", function ($editor, event) {
                onInputBoxKeyDown(event)
            });
            $chatEditorArea[0].emojioneArea.on("keypress", function ($editor, event) { onInputBoxKeyPress(event) });
            $chatEditorArea[0].emojioneArea.on("click", function ($editor, event) { onInputBoxClick(event) });
            $chatEditorArea[0].emojioneArea.on("blur", function ($editor, event) { onInputBoxBlur(event) });

            //$chatEditorArea[0].emojioneArea.on("change", function ($editor, event) { onInputBoxInput(event) });
        ...
        function updateValues() {
            ...
            //elmInputBoxEditor.html(mentionText);
        ...
        function addMention(mention) {
            ...
            var updatedMessageText = start + `<a href="#!" class="text-primary font-italic font-weight-bold" data-email= "${mention.email}" > @${mention.value} </a>` + '&nbsp;' + '';
            ...
            elmInputBox.trigger('blur');
            updateValues();

            // Set correct focus and selection
            elmInputBoxEditor.focus();
            utils.setCaratPosition(elmInputBox[0], startEndIndex);
        }

        //Gets the actual value of the text area without white spaces from the beginning and end of the value
        function getInputBoxValue() {
            return $.trim($chatEditorArea[0].emojioneArea.getText());
        ...
        function onAutoCompleteItemClick(e) {
            ...
            elmInputBox.val($chatEditorArea[0].emojioneArea.getText());
            addMention(mention);
            scrollToInput();
            return false;
        }
        ...
        function populateDropdown(query, results) {
            ...
            let elDropDownHeight = elmAutocompleteList.height;
            
            elmAutocompleteList.css('top', `-${$(elmAutocompleteList).height()}px`); //elmDropDownList height
        }
